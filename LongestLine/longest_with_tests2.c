...
int readline( char s[], int max_chars ) {
	...
	int cur_char = '\0'; // getting char?

	// count starts at 0, make sure current letter is not EOF or new line, add count for each iteration
	cur_char = getchar();
	while (cur_char != EOF){
	...
	return count ; // returning how long count is
}
...
void copy( char to[], char from[] ) {
	...
	//	Do NOT use pointers.
	int cur = 0;
	while (from[cur] != '\0') // ending put in when read line is used
	{
		to[cur] = from[cur]; // set to array
		cur++;
	}	
	to[cur] = '\0'; // end it with correct array
}
