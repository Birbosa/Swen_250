...
int test()
{
	...
	// Test 5
	int first_array[] = { 1, 2, 3, 4, 5, 6, -999 } ;	// Note that -999 is not considered as part of the array
	#define ARRAY_COUNT (6)

	result = is_pointer_in_array( first_array, first_array, ARRAY_COUNT ) ;
	assert( result == 1,
		"Points to the first position in the array so expected a 1 but got a %d",  result )
		? passcount++ : failcount++ ;
	// Test 6
	result = is_pointer_in_array( first_array, &first_array[5], ARRAY_COUNT ) ;
	assert( result == 1,
		"Points to the last position in the array so expected a 1 but got a %d",  result )
		? passcount++ : failcount++ ;
	// Test 7
	result = is_pointer_in_array( first_array, &first_array[6], ARRAY_COUNT ) ;
	assert( result == 0,
	...
	assert( p_products != NULL && p_products != first_array,
		...
		? passcount++ : failcount++ ;
	// Test 15 -- verify access to valid pointer's first entry
	assert( p_products && *p_products == 5,
		...
		? passcount++ : failcount++ ;
	// Test 16 -- verify access to valid pointer's last entry
	...
	answer = my_random() ;
	assert( answer == 54,
		"Last random number must be 54 but got a %u",  answer )
		? passcount++ : failcount++ ;
	// Test 25
	my_random() ;
	my_random() ;
